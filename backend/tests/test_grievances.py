...
from app.models import Role, User, Department, Category, Grievance
...
def setup_database():
    ...
    # Department
    dept = Department(name="Estate & Facilities", description="Facilities maintenance")
    db.add(dept)
    db.flush()

    # Category
    cat = Category(name="Plumbing", is_active=True)
    db.add(cat)
    db.flush()
...
def test_create_and_fetch_grievance():
    student_token = get_auth_token("student1@example.com")
...
def test_add_comment_and_feedback():
    student_token = get_auth_token("student1@example.com")
    headers = {"Authorization": f"Bearer {student_token}"}
    ...
    auth_token = get_auth_token("authority1@example.com")
